...
BLOCK_COLOR = Color.new(%w[orange yellow green].sample)
...
update do
  ...
  elsif Window.frames % (60 / speed) == 0
    case current_direction
    when :right
      active_squares.each { |square| square.x += GRID_SIZE }
      current_direction = :left if active_squares.last.x + active_squares.last.width >= Window.width
    when :left
      active_squares.each { |square| square.x -= GRID_SIZE }
      current_direction = :right if active_squares.first.x <= 0
    end
  end
end
...
on :key_down do
  ...
  active_squares.each do |active_square|
    unless current_line == HEIGHT - 2 || frozen_squares.has_key?("#{active_square.x},#{active_square.y + GRID_SIZE}")
      next
    end
  ...
  (0..WIDTH).each do |index|
    ...
    next unless frozen_squares.has_key?("#{x},#{y + GRID_SIZE}")
